...
void main() {
  testWidgets('an image preview renders inline and zoomable', (tester) async {
    await tester.pumpWidget(
      _wrap(
        RequirementViewerScreen(
          title: 'Consent and Waiver',
          subtitle: 'Original copy from your coordinator',
          loadPreview: () async =>
              RequirementPreview(kind: PreviewKind.image, bytes: _onePixelPng),
  ...
  testWidgets('an unsupported file falls back to a download prompt', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      _wrap(
        RequirementViewerScreen(
          title: 'Odd Format',
          subtitle: 'Original copy from your coordinator',
          loadPreview: () async => RequirementPreview(
            kind: PreviewKind.none,
            message: 'This kind of file cannot be shown here.',
          ),
          loadDownload: () async {
            downloaded = true;
            return const [1, 2, 3];
    ...
    expect(
      find.text('This kind of file cannot be shown here.'),
      findsOneWidget,
    );
    expect(
      find.widgetWithText(ElevatedButton, 'Download instead'),
      findsOneWidget,
    );
    ...
    await tester.pumpWidget(
      _wrap(
        RequirementViewerScreen(
          ...
          subtitle: 'Original copy from your coordinator',
          loadPreview: () async =>
              RequirementPreview(kind: PreviewKind.pdf, bytes: const [1, 2, 3]),
          loadDownload: () async => const [1, 2, 3],
