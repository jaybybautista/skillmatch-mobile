...
void main() {
  testWidgets('starts in the bottom-right and can be dragged elsewhere', (
    tester,
  ) async {
    ...
    expect(
      ...
      reason: 'should start on the right',
    );
    expect(
      start.dy,
      greaterThan(screen.height / 2),
      reason: 'should start near the bottom',
    );
  ...
  testWidgets('starts tracking within ~6px, while the finger is still down', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      _hostOverList(controller: controller, onTap: () {}),
    );
    await tester.pump();
    ...
    expect(
      duringDrag,
      isNot(start),
      reason: 'button should already be following after ~6px',
    );
    expect(
      controller.offset,
      0,
      reason: 'the list underneath must not scroll instead',
    );
  });

  testWidgets('stays on screen when dragged far past the edges', (
    tester,
  ) async {
    await tester.pumpWidget(_host());
    await tester.pump();
  ...
  testWidgets('glow ring is hidden at rest and lit while dragging', (
    tester,
  ) async {
    await tester.pumpWidget(_host());
    await tester.pump();

    Iterable<double> ringOpacities() => tester
        .widgetList<AnimatedOpacity>(find.byType(AnimatedOpacity))
        .map((w) => w.opacity);

    expect(
      ringOpacities().every((o) => o == 0),
      isTrue,
      reason: 'no glow at rest',
    );
    ...
    expect(
      ringOpacities().every((o) => o == 1),
      isTrue,
      reason: 'both ring layers lit while dragging',
    );
    ...
    expect(
      ringOpacities().every((o) => o == 0),
      isTrue,
      reason: 'glow fades out after release',
    );
  });

  testWidgets('the ring lights the moment the button is touched', (
    tester,
  ) async {
    // The web lights its FAB ring on hover. A phone has no hover, so a touch
    // is the equivalent: waiting for the drag threshold made a plain tap look
    ...
    expect(
      ringOpacities().every((o) => o == 1),
      isTrue,
      reason: 'lit on touch down, before any movement',
    );

    await gesture.up();
    await tester.pumpAndSettle();

    expect(ringOpacities().every((o) => o == 0), isTrue);
  });

  testWidgets('a mouse hovering over it lights the ring too', (tester) async {
    await tester.pumpWidget(_host(onTap: () {}));
    await tester.pump();
    ...
    await mouse.moveTo(tester.getCenter(find.byType(AnimatedScale)));
    await tester.pump();

    expect(
      ringOpacities().every((o) => o == 1),
      isTrue,
      reason: 'lit while hovered',
    );
    ...
    expect(
      ringOpacities().every((o) => o == 0),
      isTrue,
      reason: 'unlit once the mouse leaves',
    );
  });

  testWidgets('returns to its resting position after visiting another screen', (
    tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(
