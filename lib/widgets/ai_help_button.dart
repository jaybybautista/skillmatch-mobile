...
class _AiHelpButtonState extends State<AiHelpButton> {
  ...
  Future<void> _requestHelp() async {
    ...
    if (text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Write something first, then tap AI Help to improve it.')),
      );
      return;
    }
    ...
    try {
      final suggestion = await _service.improveText(widget.fieldLabel, text, context: widget.aiContext);
      if (!mounted) return;
      final accepted = await showAiSuggestionSheet(context, original: text, suggested: suggestion);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Something went wrong reaching the AI service. Please try again.')),
        );
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    return TextButton.icon(
      ...
      label: Text(_isLoading ? 'Thinking…' : 'AI Help', style: const TextStyle(fontSize: 12)),
...
Future<bool?> showAiSuggestionSheet(BuildContext context, {required String original, required String suggested}) {
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(color: const Color(0xFFF1F2F5), borderRadius: BorderRadius.circular(10)),
                child: SingleChildScrollView(
                  child: Text(original, style: const TextStyle(fontSize: 13, color: AppColors.textMuted)),
                ),
              ),
              const SizedBox(height: 14),
              const Text(
                'SUGGESTED',
                style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: AppColors.primary, letterSpacing: 0.4),
              ),
              const SizedBox(height: 6),
              Container(
                width: double.infinity,
                ...
                child: SingleChildScrollView(
                  child: Text(suggested, style: const TextStyle(fontSize: 13, color: AppColors.textDark)),
                ),
              ),
