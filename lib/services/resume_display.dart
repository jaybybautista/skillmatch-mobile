...
class ResumeHeaderInfo {
  const ResumeHeaderInfo({required this.fullName, this.email, this.phone, required this.addressLine, this.course});
...
ResumeHeaderInfo computeResumeHeader(Resume resume, StudentProfile? profile) {
  final basicInfo =
      resume.sections.firstWhere((s) => s.type == 'basic_info', orElse: () => _emptyBasicInfoSection).basicInfo;

  final fullName = (basicInfo?.fullName?.isNotEmpty ?? false) ? basicInfo!.fullName! : (profile?.name ?? 'Your Name');
  final email = (basicInfo?.email?.isNotEmpty ?? false) ? basicInfo!.email! : profile?.email;
  ...
  return ResumeHeaderInfo(fullName: fullName, email: email, phone: phone, addressLine: addressLine, course: profile?.course);
}

List<ResumeSection> nonBasicInfoSections(Resume resume) {
  return resume.sections.where((s) => s.type != 'basic_info').toList()..sort((a, b) => a.order.compareTo(b.order));
}
